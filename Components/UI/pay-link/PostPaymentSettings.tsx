...
const PostPaymentSettings: React.FC<PostPaymentSettingsProps> = ({
    ...
    return (
        <>
            <Box sx={{ display: "flex", flexDirection: "column", gap: isMobile ? "12px" : "16px" }}>
                <Box sx={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? "6px" : "8px" }}>
                    {hasPaymentLinkData && (
                        <Box width={currentLang === "pt" ? "210px" : "110px"}>
                            <Text
                                sx={{
                                    fontSize: isMobile ? "12px" : "15px", color: theme.palette.text.primary
                                }}>{tPaymentLink("callbackUrl")}</Text>
                        </Box>
                    )}
                    <InputField
                        ...
                        helperText={showHelpers ? tPaymentLink("callbackUrlHelper") : undefined}
                        type="url"
                        sx={{ width: "100%" }}
                    />
                </Box>
                <Box sx={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: "6px" }}>
                    {hasPaymentLinkData && (
                        <Box width={currentLang === "pt" ? "210px" : "110px"}>
                            <Text
                                sx={{
                                    fontSize: isMobile ? "12px" : "15px", color: theme.palette.text.primary
                                }}>{tPaymentLink("redirectUrl")}</Text>
                        </Box>
                    )}
                    <InputField
                        ...
                        helperText={showHelpers ? tPaymentLink("redirectUrlHelper") : undefined}
                        type="url"
                        sx={{ width: "100%" }}
                    />
                </Box>
                <Box sx={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? "6px" : "8px" }}>
                    ...
                    <InputField
                        ...
                        helperText={showHelpers ? tPaymentLink("webhookUrlHelper") : undefined}
                        type="url"
                        sx={{ width: "100%" }}
                    />
                </Box>
