...
import { PaymentLinkHeaderProps } from "@/utils/types/paymentLink";

const DisabledNotice: React.FC<{ isMobile: boolean; tPaymentLink: (key: string) => string }> = ({ isMobile, tPaymentLink }) => (
    <Box
        sx={{
...
const PaymentLinkHeader: React.FC<PaymentLinkHeaderProps> = React.memo(
    ({ tPaymentLink, paymentLinkData, disabled, isMobile, count, truncateByWords }) => (
        <>
            <Box
                sx={{
                    display: "flex",
            ...
            {disabled && <DisabledNotice isMobile={isMobile} tPaymentLink={tPaymentLink} />}
            <Box
                sx={{
                    height: "1px",
