...
const CryptoItemCard: React.FC<CryptoItemCardProps> = React.memo(
    ({ item, isMobile, walletNotSetUp, paymentSettings, setPaymentSettings, tPaymentLink, isLarge, isSmall }) => (
        <Grid item xs={useMediaQuery("(min-width:1200px) and (max-width:1299px)")? 6 : isLarge ? 4 : isSmall ? 6 : 12} key={item.label} >
            <Box
                onClick={() => {
                ...
                sx={{
                    ...
                    maxWidth: "326px",
                    border: `1px solid ${paymentSettings.acceptedCryptoCurrency.includes(item.label)
                    ...
                    borderRadius: "14px",
                    padding: isMobile ? "10px" : "18px 10px",
                    display: "flex",
                    alignItems: "center",
            ...
            >
                ...
                <Text sx={{ fontSize: "12px", color: "#676B7E" }}>{tPaymentLink("stable")}</Text>
